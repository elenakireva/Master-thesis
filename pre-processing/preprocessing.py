...
def make_sentences_df(input_dataset):
    ...
    for number in df['line_merge_number'].unique():
        ...
        if len(tobe_merged_data) <= 1:
            df2 = df2._append(tobe_merged_data)
        else:
...
if __name__ == "__main__":
    ...
    preprocessed_df = preprocess_dataset("protocol_dementia.csv", False, True, False)
    df_dementia = make_sentences_df(preprocessed_df)
    df_dementia.to_csv("dementia_preprocessed.csv")

    preprocessed_df = preprocess_dataset("protocol_control.csv", False, False, True)
    df_control = make_sentences_df(preprocessed_df)
    df_control.to_csv("control_preprocessed.csv")
